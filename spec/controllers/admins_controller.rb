...
RSpec.describe Admins::AdminsController, type: :controller do
  ...
  describe "POST create action" do
    let!(:params)   {ActionController::Parameters.new({user: {email: 'admin@gmail.com', accelerator_id: accelerator.id}})}
    let!(:params_2) {ActionController::Parameters.new({user: {email: 'admin2@gmail.com', accelerator_id: accelerator_2.id}})}
    ...
    it "return error in json with status forbidden if current super_admin try to create admin for an accelerator doesnt belong to him" do
      request.headers.merge!({ "Accelerator-Id": "#{accelerator.id}"})
      sign_in super_admin
