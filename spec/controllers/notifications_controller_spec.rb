...
RSpec.describe NotificationsController, type: :controller do
    ...
    let!(:category)           { create(:category, assessment_id: assessment.id) }
      let!(:sub_category)     { create(:sub_category, category_id: category.id) }
        let!(:stage)          { create(:stage, sub_category_id: sub_category.id) }
          let!(:tasks)        { create_list(:task, 2, stage_id: stage.id, task_users_attributes: [{user_id: member.id}, {user_id: team_lead.id}, {user_id: super_admin.id, creator: true}, {user_id: admins.first.id}]) }
          let!(:tasks_2)      { create_list(:task, 2, stage_id: stage.id, task_users_attributes: [{user_id: member.id}, {user_id: admins.last.id, creator: true}, {user_id: team_lead.id}]) }
  ...
  describe "GET index action" do
    ...
    it "return notifications for current team_lead" do
      request.headers.merge!({ "Accelerator-Id": "#{accelerator.id}"})
      sign_in team_lead
      get :index
      expect(parse_json(response.body)[0]).to eq(["current_page", 1])
  ...
  describe "PUT mark_as_readed_all action" do
    ...
    it "return successful message in json format and update all notifications read statuses to true for current user if team_lead authenticated" do
      request.headers.merge!({ "Accelerator-Id": "#{accelerator.id}"})
      sign_in team_lead
      expect(Notification.where(user_id: team_lead.id, read: false).count).to eq(4)
      put :mark_as_readed_all
      expect(Notification.where(user_id: team_lead.id, read: false).count).to eq(0)
      expect(response.body).to eq({message: "Successfully update read status to true for all user notifications"}.to_json)
      expect(response.content_type).to eq('application/json; charset=utf-8')
      expect(response).to have_http_status(:success)
    end
  ...
  describe "PUT mark_as_readed action" do
    ...
    let!(:notification_2) { create(:notification, task_id: tasks.first.id, user_id: team_lead.id) }
    ...
    it 'return error with status 403 if notification doesnt belong to the team_lead' do
      request.headers.merge!({ "Accelerator-Id": "#{accelerator.id}"})
      sign_in team_lead
